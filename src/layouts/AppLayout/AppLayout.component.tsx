...
const AppLayout: FC = () => {
	...
	const routeTitle = useMemo(() => {
		if (!routeLabel) return `GoDevRun - All-in-one Toolkit for Developers`

		return `${routeLabel} | GoDevRun`
	}, [routeLabel])

	const routeDescription = useMemo(() => {
		if (toolRoute && toolRoute.description) return toolRoute.description

		return `Stop pasting your code on random websites, GoDevRun centralizes all your go-to utilities in one convenient location with 45+ carefully crafted tools.`
	}, [toolRoute])
			...
			<Layout className='flex flex-col h-lvh'>
				<Layout.Header className={`${theme === 'light' ? 'bg-white' : ''} p-0 z-10 shadow-sm flex items-center`}>
					<Link to='/' className='items-center gap-2 w-[310px] p-7 hidden xl:flex'>
						...
						<Typography.Title level={2} className='text-xl mb-0'>GoDevRun</Typography.Title>
					</Link>
					...
					<div className='grow flex items-center justify-between'>
						...
						<div className='mx-8 flex items-center gap-4'>
							<Button className='hidden sm:block' icon={<GithubOutlined />} type='text' size='large' href='https://github.com/sunnydanu/godev.run/issues' target='_blank'>Report an issue</Button>
							<Switch
